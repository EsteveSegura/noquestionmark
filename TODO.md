# ChatCrimes — TODO: Estrategia de Contenido

## Nuevos crimes por crear

- [x] **The lonely "hi"** (sin contexto) → `/no-hello`
- [ ] **Voice note en vez de texto** → `/no-voice-notes`
- [ ] **Wall of text sin formato** → `/no-wall-of-text`
- [ ] **@channel / @here innecesario** → `/no-unnecessary-mentions`
- [ ] **Mensajes partidos en 10 líneas** → `/no-message-splitting`
- [ ] **"Can I ask you a question?"** → `/just-ask`
- [ ] **Responder "ok" a algo complejo** → `/no-dry-ok`
- [ ] **Screenshot de texto** → `/no-screenshot-of-text`
- [ ] **Mensajes a las 3AM esperando respuesta** → `/respect-time-zones`
- [ ] **Borrar mensajes sin avisar** → `/no-ghost-deleting`
- [ ] **GIF como única respuesta** → `/no-gif-only-reply`
- [ ] **"As per my last message" pasivo-agresivo** → `/no-passive-aggressive`
- [ ] **Reenviar sin contexto** → `/no-forward-without-context`
